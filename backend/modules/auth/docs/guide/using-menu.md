...
Return of `backend\modules\auth\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `backend\modules\auth\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `backend\modules\auth\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
```php
<?php
user backend\modules\auth\components\Helper;
