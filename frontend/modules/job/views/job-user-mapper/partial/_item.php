...
?>
    ...
    <div class="jobsbox">
        <?php if($unreadMsgCount > 0 ){?>
            <div class="counter">
                <a href="<?=Url::to('/user/user-msg')?>">    <?=$unreadMsgCount;?>
                </a>
            </div>
        <?php } ?>
        <div class="jobsimg">
            <?php echo Html::a(EasyThumbnailImage::thumbnailImg($job->logo, 350, 300) ,['job-item/view','id' => $model->job_id]);?>
        </div>
        <h3>
    ...
    ?>
    <span>Project Type :</span> <?= $job->getJobFieldValue('project-type');?>	<br/><span>Role Type :</span> <?= $job->getJobFieldValue('role-type');?>	<br/><span>Location :</span> <?= $job->location;?>
    <br/><span>Project Time :</span> <?php if($diff->y > 0){
        echo $diff->y . ' yr(s) ';}
        echo $diff->m . ' month(s)';?>
        <br/><span>Compensation Type :</span>
    <?= $job->getJobFieldValue('compensation-type');?>
    </div>
    <?php } ?>
        <div class="buttondiv">
            <?php
            if($model->status == "Approved" || $model->status == "Booked"){ ?>
                <?= Html::a('Pass', ['/job/job-item/talent-status','status' => 'Declined'],['class' => 'btn btn-primary respond-rolecall',		'job_id'=>$model->job_id,'user_id'=>$model->user_id])?>
            <?php }
            else if($model->status == "Declined") { ?>
                <?= Html::a('Select', ['/job/job-item/talent-status','status' => 'Approved'],['class' => 'btn btn-primary respond-rolecall',			'job_id'=>$model->job_id,'user_id'=>$model->user_id])?>
            <?php }
