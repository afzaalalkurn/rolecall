...
class Notification extends Component{
    ...
    public static function handler(NotificationEvent $event){
        ...
        if($event->status) {
            switch($event->status){
                ...
                case NotificationEvent::NOTIFICATION_SEND:
                    ...
                    break;
                case NotificationEvent::NOTIFICATION_MESSAGE:
                    $action_text = $event->status;
                    $msg_id   = $event->msg_id ? $event->msg_id: null;
                    if($msg_id){
                        $modelMessage = UserMsg::findOne($msg_id);
                        $sender_id =  $modelMessage->sender_id;
                        $modelUser = User::findOne($sender_id);
                        $modelUserProfile = $modelUser->userProfile;
                        $recipient_name_msg = ucwords($modelUserProfile->getName());
                    }
                    break;
        ...
        if($user_mapper_id){
            ...
            if($role == "Director"){ $recipient_id = $sender_id; }
